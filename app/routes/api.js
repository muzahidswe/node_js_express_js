...
const { uploaddir,
  ...
  bulkKYCApproveUpload,
  uploadOutletInfo,
  fileUploadInterestSettings,
  fileUploadGeneric
...
const ekyc = require("../controllers/eKycController");
const lbc = require("../controllers/leaderBoardController");
const loanLossClassification = require("../controllers/loanLossController");
...
const uploadBulkKYCApprove = multer({
  storage: bulkKYCApproveUpload('kyc_file')
});

const uploadOutletDetailsInfo = multer({
  storage: uploadOutletInfo('outlet_file')
});
...
const uploadRetailerDoc = multer({
  ...
  storage: uploadRetailerDocsConfig("unnoti_outlet_documents")
});

const uploadZip = multer({
  // storage: uploadRetailerDocsConfig("account_form")
  storage: uploadRetailerDocsConfig("unnoti_account_form")
});
...
const uploadInterestSettings = multer({
  storage: fileUploadInterestSettings('interest-settings')
});
...
router.post(
	"/upload-interest-settings",
	uploadInterestSettings.single("file"),
	file.uploadInterestSettingsFile
);
...
router.post("/change-sms-gateway", kyc.changeSmsGateway);
router.get("/sms-gateway-list", kyc.SmsGatewayList);
...
// by Mahfuz
router.get("/kyc-document-status-point/:point_id/:doc_ready", kyc.kycDocumentStatusByPoint);
...
//router.get("/total_outstanding_calculation_daily", interestCalculation.calculateTotalOutstandingDaily);
...
router.get("/credit-outlet-list", support.supportCreditOutletList);
router.get("/otp-check-log", support.OtpCheckLog);

// ekyc © muzahid
router.post("/ekyc-nid-checking", ekyc.nidInfoChecking);
router.get("/ekyc-nid-details/:id?", ekyc.nidInfoDetails);
router.post("/ekyc-nid-list", ekyc.nidInfoList);
router.post("/download-ekyc-list", ekyc.downloadeKycOutletList);

// Customer Report © muzahid
router.post("/download_customer_report", report.downloadCustomerReport);

router.get("/check-live-sync-status/:id_point?", disbursement.checkLiveSyncStatus);
router.get("/get-interest-settings/:id_point?", disbursement.getInterestSettings);

// outstanding Report Download © muzahid
router.post("/outstanding_report_download", report.outstandingReportDownload);

router.get("/retailer-nid-information", kyc.retailerNidInformation);

// Loan Account Number & Customer ID Adjustment © muzahid
router.get("/pending-loan-customer-id", kyc.pendingLoanCustomerId);
router.get("/loan-customer-id-adjustment", kyc.loanCustomerIdAdjustment);

// Bulk KYC Approve © muzahid
router.post(
  "/upload-bulk-kyc-approve",
  uploadBulkKYCApprove.single("file"),
  file.uploadBulkKYCApproveFile
);

router.post(
  "/upload-retailer-nid-info",
  uploadOutletDetailsInfo.single("file"),
  file.uploadOutletDetailsInfoFile
);

// leaderboard © rokib hasnat
router.get("/leader-board", lbc.getDh);

// Live Update © Mahfuz Ahmad
router.get('/live-update', report.registrationLiveUpdate);

// Outlet By Id Point © Mahfuz Ahmad
router.post("/get-outlet-by-point", user.getOutletByPoint);

// Get DH FI mapping © Mahfuz Ahmad
router.post('/get-dh-by-fi', bank.getDhidsByFI);


// Develop by moin
router.post("/get-nid-master-data",report.getNidMasterData);
router.post("/get-nid-master-data-report",report.getNidMasterDataReport);

router.post("/otp_log", support.otpLog);


// retailer phone number change start 
const uploadRetailerPhoneNumberChange = multer({
  storage: fileUploadGeneric('retailer_phone_number_change')
});


router.post(
	"/bulk-upload-retailer-phone-number",
	uploadRetailerPhoneNumberChange.single("file"),
	file.uploadRetailerPhoneNumberChangeBulk
);

router.post("/single-upload-retailer-phone-number", kyc.uploadRetailerPhoneNumberChangeSingle)
// retailer phone number change start 


// v2 app intregration by mahfuz
router.post("/outlet-credit-details", cc.getOutletCreditDetails);
router.post("/download-nid-bulk", kyc.downloadNidBulk);

// loan loss
// router.get("/calculate-provision-wise-outlet-classification",loanLossClassification.determindClassificationWiseOutlets);
router.post("/get-outlet-loan-loss-data",loanLossClassification.outletLoanLossData);
router.post("/get-summery-outlet-loan-loss-data",loanLossClassification.summeryOfOutletLoanLossData);
router.post("/get-outlet-loan-loss-report",loanLossClassification.outletLoanLossReport);
